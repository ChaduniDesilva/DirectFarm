...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'upload_photo') {
    if (!empty($_FILES['profile_photo']['name'])) {
        ...
        if (in_array($ext, ['jpg','jpeg','png','webp'])) {
            // Delete old photo
            $oldPic = $db->query("SELECT profile_pic FROM users WHERE id=$uid")->fetch_assoc()['profile_pic'];
    ...
    header("Location: dashboard.php?tab=" . ($_GET['tab'] ?? 'orders'));
    exit;
}
...
<head>
    ...
    <style>
        ...
        /* Sidebar Profile */
        .sb-profile-section { padding:28px 20px 20px; border-bottom:1px solid rgba(255,255,255,0.08); text-align:center; margin-bottom:10px; }
        .sb-avatar-wrap { position:relative; width:72px; height:72px; margin:0 auto 12px; cursor:pointer; }
        .sb-avatar-img { width:72px; height:72px; border-radius:50%; object-fit:cover; border:3px solid #56ad58; display:block; }
        .sb-avatar-initials { width:72px; height:72px; border-radius:50%; background:linear-gradient(135deg,#009933,#006622); display:flex; align-items:center; justify-content:center; font-size:26px; font-weight:700; color:white; border:3px solid #56ad58; }
        .sb-avatar-overlay { position:absolute; inset:0; border-radius:50%; background:rgba(0,0,0,0.45); display:flex; align-items:center; justify-content:center; opacity:0; transition:0.2s; color:white; font-size:18px; }
        .sb-avatar-wrap:hover .sb-avatar-overlay { opacity:1; }
        .sb-user-name { color:white; font-weight:700; font-size:15px; margin-bottom:4px; }
        .sb-user-role { color:rgba(255,255,255,0.4); font-size:11px; letter-spacing:2px; font-weight:600; }
</style>
</head>
<body>
...
<div class="dashboard-layout">
    ...
    <div class="sidebar">
        ...
        <div class="sb-profile-section">
            <div class="sb-avatar-wrap" onclick="document.getElementById('sidebarPhotoInput').click()" title="Change photo">
                <?php if (!empty($user['profile_pic']) && file_exists('../'.$user['profile_pic'])): ?>
                    <img src="../<?= htmlspecialchars($user['profile_pic']) ?>?t=<?= time() ?>" class="sb-avatar-img">
                <?php else: ?>
                    <div class="sb-avatar-initials"><?= strtoupper(substr($user['name'],0,1)) ?></div>
                <?php endif; ?>
                <div class="sb-avatar-overlay"><i class="fa-solid fa-camera"></i></div>
            </div>
            <form method="POST" enctype="multipart/form-data" id="sidebarPhotoForm">
                <input type="hidden" name="action" value="upload_photo">
                <input type="file" id="sidebarPhotoInput" name="profile_photo" accept="image/*" style="display:none;" onchange="document.getElementById('sidebarPhotoForm').submit()">
            </form>
            <div class="sb-user-name"><?= htmlspecialchars($_SESSION['user_name']) ?></div>
            <div class="sb-user-role">🛒 CONSUMER</div>
        </div>
